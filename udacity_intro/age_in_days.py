...
def daysBetweenDates(y1, m1, d1, y2, m2, d2):
    ...
    if(m2 > m1):
        ...
        for month in range(m1+1,m2):
            # # # # print month
            days += daysOfMonth(y2,month)
            # # # # print days
        endOfStartMonth = daysOfMonth(y2,m1)
        days += endOfStartMonth - d1
        days += d2
    elif(m2 < m1):
        # print 'm2<'
        for month in range(m2+1,m1):
            # print 'month ' + str(month)
            # print days
            # print daysOfMonth(y2,month)
            days -= daysOfMonth(y2,month)
        # print days

        endOfEndMonth = daysOfMonth(y2,m2)
        # print '?'
        # print endOfEndMonth - d2
        days -= endOfEndMonth - d2
        # print days
